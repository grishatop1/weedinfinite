...
class Terrain:
    ...
    def place(self):
        ...
        pygame.draw.rect(
            game.win, BLACK, (*game.c.offset(x_tile, y_tile), TILE, TILE), 1
        )
        ...
        if game.L_click:
            ...
            if (x_tile, y_tile) not in self.loaded_chunks[(x_chunk, y_chunk)]:
                self.loaded_chunks[(x_chunk, y_chunk)][(x_tile, y_tile)] = {
                    "id": self.selected_texture
                }
        if game.R_click:
            mox, moy = game.c.reOffset(game.xm, game.ym)
            x_tile, y_tile = mox // TILE * TILE, moy // TILE * TILE
    ...
    def draw(self):
        ...
        for x, y in self.getTilesInScreen():
            ...
            game.win.blit(
                game.tx.getTextureImage(self.loaded_chunks[(cx, cy)][(x, y)]["id"]),
                (xo, yo),
            )
...
class Player:
    ...
    def draw(self):
        pygame.draw.rect(
            game.win, self.color, (*game.c.offset(self.x, self.y), self.w, self.h)
        )
...
class Worker:
    def __init__(self, func):
        ...
        self.proc = multiprocessing.Process(
            target=self.func, daemon=True, args=(self.lockin, self.lockout)
        )
        self.proc.start()
    ...
    def main(self):
        ...
        while self.run:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if event.button == 4:
                        ...
                        while not self.tx.getBlockById(self.t.selected_texture)[
                            "obtainable"
                        ]:
                            self.t.selected_texture += 1
            ...
            pos_txt = self.font.render(
                f"Block X:{int(self.p.x // TILE)}  Y:{int(self.p.y // TILE)}",
                True,
                BLACK,
            )
            self.win.blit(pos_txt, (0, 30))
            posc_txt = self.font.render(
                f"Chunk X:{int(self.p.x // CHUNK)}  Y:{int(self.p.y // CHUNK)}",
                True,
                BLACK,
            )
            self.win.blit(posc_txt, (0, 60))
            tx_txt = self.font.render(
                f"Texture: {self.tx.blocks[str(self.t.selected_texture)]['name']}",
                True,
                BLACK,
            )
